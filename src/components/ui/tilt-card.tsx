...
export function TiltCard({
  ...
  const effectiveTiltLimit = isCoarsePointer ? tiltLimit * 0.45 : tiltLimit;
  const effectiveScale = isCoarsePointer ? 1 + (scale - 1) * 0.35 : scale;
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      const xRot = (py - 0.5) * (effectiveTiltLimit * 2) * dir;
      const yRot = (px - 0.5) * -(effectiveTiltLimit * 2) * dir;
      setTransform(
    ...
    [effectiveTiltLimit, effectiveScale, perspective, dir, spotlight],
  );
  ...
  return (
    <div
      ...
      className={cn("will-change-transform relative overflow-hidden h-full", className)}
      style={{
        transform,
